 ...
 */
void Uart_Init(
  UART_HandleTypeDef * huart, uint8_t * Rx_Buffer, uint16_t Rx_Buffer_Size,
  UART_Call_Back Callback_Function)
{
  if (huart->Instance == USART1) {
    ...
    // <--- 新增：绑定发送缓冲区
    UART1_Manage_Object.Tx_Buffer = UART1_Tx_Data;
    UART1_Manage_Object.Tx_Buffer_Size = 256;
  }
    ...
    // <--- 新增
    UART2_Manage_Object.Tx_Buffer = UART2_Tx_Data;
    UART2_Manage_Object.Tx_Buffer_Size = 256;
  }
    ...
    // <--- 新增
    UART3_Manage_Object.Tx_Buffer = UART3_Tx_Data;
    UART3_Manage_Object.Tx_Buffer_Size = 256;
  }
    ...
    UART4_Manage_Object.Tx_Buffer = UART4_Tx_Data;  // 记得加上之前修补的这行
    UART4_Manage_Object.Tx_Buffer_Size = 256;       // 还有这行
  }
    ...
    UART5_Manage_Object.Tx_Buffer = UART5_Tx_Data;  // 记得加上之前修补的这行
    UART5_Manage_Object.Tx_Buffer_Size = 256;       // 还有这行
  }
    ...
    // <--- 新增
    UART6_Manage_Object.Tx_Buffer = UART6_Tx_Data;
    UART6_Manage_Object.Tx_Buffer_Size = 256;
  }
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef * huart, uint16_t Size)
{
  ...
  if (huart->Instance == USART1) {
    UART1_Manage_Object.Callback_Function(
      UART1_Manage_Object.Rx_Buffer, UART1_Manage_Object.Rx_Buffer_Size);
    HAL_UARTEx_ReceiveToIdle_DMA(
      huart, UART1_Manage_Object.Rx_Buffer, UART1_Manage_Object.Rx_Buffer_Size);
  }
  else if (huart->Instance == USART2) {
    UART2_Manage_Object.Callback_Function(
      UART2_Manage_Object.Rx_Buffer, UART2_Manage_Object.Rx_Buffer_Size);
    HAL_UARTEx_ReceiveToIdle_DMA(
      huart, UART2_Manage_Object.Rx_Buffer, UART2_Manage_Object.Rx_Buffer_Size);
  }
  else if (huart->Instance == USART3) {
    UART3_Manage_Object.Callback_Function(
      UART3_Manage_Object.Rx_Buffer, UART3_Manage_Object.Rx_Buffer_Size);
    HAL_UARTEx_ReceiveToIdle_DMA(
      huart, UART3_Manage_Object.Rx_Buffer, UART3_Manage_Object.Rx_Buffer_Size);
  }
  else if (huart->Instance == UART4) {
    UART4_Manage_Object.Callback_Function(
      UART4_Manage_Object.Rx_Buffer, UART4_Manage_Object.Rx_Buffer_Size);
    HAL_UARTEx_ReceiveToIdle_DMA(
      huart, UART4_Manage_Object.Rx_Buffer, UART4_Manage_Object.Rx_Buffer_Size);
  }
  else if (huart->Instance == UART5) {
    UART5_Manage_Object.Callback_Function(
      UART5_Manage_Object.Rx_Buffer, UART5_Manage_Object.Rx_Buffer_Size);
    HAL_UARTEx_ReceiveToIdle_DMA(
      huart, UART5_Manage_Object.Rx_Buffer, UART5_Manage_Object.Rx_Buffer_Size);
  }
  else if (huart->Instance == USART6) {
    UART6_Manage_Object.Callback_Function(
      UART6_Manage_Object.Rx_Buffer, UART6_Manage_Object.Rx_Buffer_Size);
    HAL_UARTEx_ReceiveToIdle_DMA(
      huart, UART6_Manage_Object.Rx_Buffer, UART6_Manage_Object.Rx_Buffer_Size);
  }
  // else if (huart->Instance == UART7)
  // {
